...
3. Glob으로 `./til/{카테고리}/*.md`를 조회하여 마지막 번호를 확인하고, 다음 번호로 `./til/{카테고리}/{NN}-{주제슬러그}.md`에 저장한다
...
│   └── 01-ownership.md
└── devops/
    └── 01-docker-network.md
```

- 기본 경로: `./til/{카테고리}/{NN}-{주제슬러그}.md`
- `{NN}`: 카테고리 내 순번 (01, 02, 03...). 기존 파일을 확인해서 다음 번호를 부여한다
- 카테고리 폴더가 없으면 자동 생성 (첫 파일은 01번)
...
```markdown
## TIL
- [[til/javascript/01-closure|클로저(Closure)]]
```
...
### javascript
1. [[til/javascript/01-closure|클로저(Closure)]]
2. [[til/javascript/02-event-loop|이벤트 루프(Event Loop)]]

### devops
1. [[til/devops/01-docker-network|Docker 네트워크]]
```
