...
물리적으로 제거되지 않고 xmax만 설정된다. 실제 정리는 [[VACUUM]]이 담당한다.
...
[[트랜잭션 격리 수준(Isolation Level)]]에 따라 스냅샷을 갱신하는 시점이 달라진다.
...
4. [[WAL(Write-Ahead Logging)|WAL]] 레코드 기록
...
MVCC의 부작용으로 **죽은 튜플(dead tuple)**이 계속 쌓인다. 이를 정리하는 것이 [[VACUUM]]이다.
