...
통합 서비스 태깅이 없으면 같은 서비스인데 [태그](til/datadog/tagging.md)명이 제각각이라 연결이 안 된다:
...
4. **원클릭 전환**: [메트릭](til/datadog/metrics.md) → [로그](til/datadog/log-management.md) → [트레이스](til/datadog/apm-distributed-tracing.md)를 동일한 컨텍스트로 즉시 전환
...
> 2. `env:production service:checkout`으로 [대시보드](til/datadog/dashboards.md) 필터링 → version별 에러율 비교
