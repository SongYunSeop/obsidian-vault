...
[메인 에이전트](til/claude-code/agent.md)가 Task 도구를 통해 생성하는 독립 실행 단위다. 각 서브에이전트는 **별도의 컨텍스트 윈도우**에서 커스텀 시스템 프롬프트, 제한된 도구, 독립 권한으로 동작한다.

핵심 가치 4가지:

- **컨텍스트 보존** — 탐색/구현 결과가 메인 대화를 오염시키지 않음
- **도구 제한** — 특정 도구만 허용해서 안전하게 격리
- **비용 제어** — Haiku 같은 저렴한 모델로 라우팅 ([Cost 최적화](til/claude-code/cost-optimization.md))
...
| `skills` | - | 시작 시 주입할 [스킬](til/claude-code/skill.md) (부모에서 상속 안됨) |
| `mcpServers` | - | 사용 가능한 [MCP](til/claude-code/mcp.md) 서버 |
| `hooks` | - | 전용 [라이프사이클 훅](til/claude-code/hooks.md) |
...
| 4 (최저) | [플러그인](til/claude-code/plugin.md) 에이전트 | 플러그인 `agents/` | 플러그인 활성화된 곳 |
...
| 방식 | 설명 |
|------|------|
| **포그라운드** | 메인 대화 블로킹, [권한](til/claude-code/permission-mode.md) 요청/질문 가능 |
...
| 비교 항목 | 서브에이전트 | [Agent Teams](til/claude-code/agent-teams.md) |
|-----------|------------|-------------|
...
> **Agent Teams**: 장시간 협업, [컨텍스트 윈도우](til/claude-code/context-management.md)를 초과하는 큰 작업, 에이전트 간 소통 필요 시
